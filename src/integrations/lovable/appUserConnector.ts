/**
 * Helpers do App User Connector (OAuth por usuário final).
 *
 * SOMENTE SERVIDOR. Lê segredos de `process.env` e fala direto com o gateway
 * de conectores da Lovable. Nunca importe este arquivo em componentes/rotas
 * do cliente — isso vazaria credenciais de todos os usuários do workspace.
 */
...
function requireApiKey(): string {
  ...
  if (!key) {
    throw new Error(
      "LOVABLE_API_KEY não está configurada. As chamadas do App User Connector exigem esse token no servidor.",
    );
  }
  return key;
}
...
export interface AppUserOAuthAuthorizeParams {
  ...
  clientAPIKey: string;
  returnUrl: string;
  /** Reconexão: chave lovack_* já existente do usuário. */
...
export async function authorizeAppUserOAuth(
  ...
  if (!res.ok) {
    throw new Error(`Falha ao iniciar o login Microsoft (${res.status}): ${text || res.statusText}`);
  }
  ...
  try {
    ...
    throw new Error(`Resposta inválida ao iniciar o OAuth: ${text.slice(0, 200)}`);
  }
  if (!body.authorization_url) throw new Error("Resposta do OAuth sem authorization_url");
...
export interface CallAsAppUserParams {
  ...
  connectionAPIKey: string;
  connectorId: string;
  /** Caminho sob o conector, ex.: "/me/drive/root/children". */
  path: string;
  init?: RequestInit;
}
...
/** Troca o `code` de uso único do retorno OAuth pela chave de conexão do usuário. */
export async function exchangeAppUserOAuthCode(
  ...
  const text = await res.text();
  if (!res.ok) {
    throw new Error(`Falha ao concluir o login Microsoft (${res.status}): ${text || res.statusText}`);
  }
  ...
  try {
    ...
    throw new Error(`Resposta inválida na troca do código: ${text.slice(0, 200)}`);
  }
  if (!body.api_key) throw new Error("Troca do código não retornou api_key");
  if (!body.connector_id) throw new Error("Troca do código não retornou connector_id");
