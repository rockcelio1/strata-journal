...
import { Pencil, X, Plus, GripVertical, ListChecks } from "lucide-react";
import {
...
import {
  listListaTarefas, upsertListaTarefaItem, deleteListaTarefaItem, reorderListaTarefas,
...
const EMPTY_FORM: FormState = { codigo: "", nome: "", is_etapa: false, percentual: 0, ativo: true };
...
function ListaTarefasPage() {
  ...
  const { data = [] } = useQuery({ queryKey: ["lista_tarefas"], queryFn: () => listFn() });
  const items = data as Item[];

  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState<"todas" | "etapas" | "tarefas">("todas");
  const [showOnlyEtapas, setShowOnlyEtapas] = useState(false);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((it) => {
      ...
      if (showOnlyEtapas && !it.is_etapa) return false;
      if (hideCompleted && it.percentual >= 100) return false;
      if (!q) return true;
      return it.codigo.toLowerCase().includes(q) || it.nome.toLowerCase().includes(q);
    });
  }, [items, search, filter, showOnlyEtapas, hideCompleted]);

  const totals = useMemo(() => {
    const total = items.length;
    const naoIniciada = items.filter((i) => i.percentual === 0).length;
    const emAndamento = items.filter((i) => i.percentual > 0 && i.percentual < 100).length;
    const concluida = items.filter((i) => i.percentual >= 100).length;
  ...
  const reorder = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["lista_tarefas"] }),
    onError: (e: any) => toast.error(e?.message ?? "Erro"),
  });
  ...
  function onDragEnd(e: DragEndEvent) {
    ...
    // Preserve items outside the current filter at the top by combining
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Catálogo global de etapas e tarefas da empresa</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_220px_auto] gap-3 items-center">
          ...
          <div className="flex flex-col gap-1 text-sm">
            <label className="flex items-center gap-2 cursor-pointer">
              <Checkbox checked={showOnlyEtapas} onCheckedChange={(v) => setShowOnlyEtapas(!!v)} />
        ...
        <DialogContent>
          ...
          <div className="grid grid-cols-1 gap-3">
            <div className="grid grid-cols-[120px_1fr] gap-3">
              <div>
                <Label>Código</Label>
                <Input value={form.codigo} onChange={(e) => setForm({ ...form, codigo: e.target.value })} placeholder="1.0" />
              </div>
              <div>
                <Label>Nome</Label>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={save.isPending || !form.codigo || !form.nome}
...
function Row({
  item, reordering, onEdit, onDelete,
}: {
  item: Item; reordering: boolean; onEdit: () => void; onDelete: () => void;
}) {
  ...
  return (
    <li
      ...
      style={style}
      className={cn(
        "grid grid-cols-[auto_60px_1fr_120px_auto] items-center gap-3 px-3 md:px-4 py-2.5 text-sm",
